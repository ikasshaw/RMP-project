...
function containsObstacles = processCellLayers(c)
    ...
    for layer = c.layers
        R = c.rects{layer};
        R.state = classifyIntersection(child.ps, obsUnion);
        
        c.layerContainment(end+1) = ~strcmp(R.state, 'Empty');
    end
    ...
    R.state = classifyIntersection(child.ps, obsUnion);
    R.depth = curRect.depth + 1;
    R.subdivide = strcmp(child.state, 'Mixed');
